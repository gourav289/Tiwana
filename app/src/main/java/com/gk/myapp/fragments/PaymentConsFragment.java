...
import com.gk.myapp.activities.cons.PaymentHistoryCustActivity;
 ...
 */
public class PaymentConsFragment extends BaseFragment implements View.OnClickListener, DatePickerDialog.OnDateSetListener {
...
//        getPartyList();
    ...
    private void getIds(View v) {
        linChequeNo = (LinearLayout) v.findViewById(R.id.lin_cheque_number);
        linTransactionId = (LinearLayout) v.findViewById(R.id.lin_transaction_id);
        edAmount = (EditText) v.findViewById(R.id.ed_amount);
        edTransactionId = (EditText) v.findViewById(R.id.ed_transaction_id);
        edChequeno = (EditText) v.findViewById(R.id.ed_cheque_no);
        txtDate = (TextView) v.findViewById(R.id.txt_date);
        spnPartyName = (Spinner) v.findViewById(R.id.spn_party_name);
        spnPaymentMode = (Spinner) v.findViewById(R.id.spn_payment_mode);
        btnAdd = (Button) v.findViewById(R.id.btn_add_payment);
...
//        spnPartyName.setAdapter(partyListAdapter);
        spnPartyName.setVisibility(View.GONE);

        mActivity.setRightButton(true, R.mipmap.ic_history, new BaseActivityTabsCons.HeaderButtonClick() {
            @Override
            public void onHeaderButtonClicked(int id) {
                Intent mIntent = new Intent(mActivity, PaymentHistoryActivity.class);
...
//                });
    ...
    private void showDatePicker() {
        ...
        dpd.show(mActivity.getFragmentManager(), "Select Date");
    }
...
//    }
    ...
    private void addPayment() {
        if (U.isConnectedToInternet(getCont())) {
            U.showProgress(getCont());
            ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
            Call<CommonSuccessModel> call = apiService.addPayment(getPreferences().getStringPref(P.USER_ID),amount,paymentMode,transactionId,date,getPreferences().getStringPref(P.USER_ID));
