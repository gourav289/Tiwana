...
/**
 * Created by Gk on 14-10-2017.
 */
public class SelectSoActivity extends BaseActivity {
    private ListView lv;
    ...
    private String year, half, title, asmId, from;

    @Override
    protected String setHeaderTitle() {
        return getString(R.string.select_so);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getIds();
        setListeners();
        if (from != null && from.equals(C.ASM))
            getSoForAsm();
        else
            getSo();
    }
    ...
    private void getIds() {
        ...
        asmId = getIntent().getStringExtra("asm_id");
    ...
    private void setListeners() {
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent mIntent = new Intent(SelectSoActivity.this, TargetDetailsActivity.class);
                mIntent.putExtra("title",txtHeading.getText().toString());
                mIntent.putExtra("year",year);
                mIntent.putExtra("half",half);
                mIntent.putExtra("so_id", mList.get(position).getId());
    ...
    private void getSo() {
        if (U.isConnectedToInternet(getCont())) {
            ...
            Call<SelectAsmResponse> call = apiService.getTargetSoForGm(getPreferences().getStringPref(P.USER_ID), half, year, asmId);
            call.enqueue(new Callback<SelectAsmResponse>() {
                ...
                public void onResponse(Call<SelectAsmResponse> call, Response<SelectAsmResponse> response) {
                    ...
                    if (data.getStatus().equals("1")) {
                        ...
                        mList.addAll(data.getData().getSoList());
                    } else {
    ...
    private void getSoForAsm() {
        if (U.isConnectedToInternet(getCont())) {
            U.showProgress(getCont());
            mList.clear();
            ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
            Call<SelectAsmResponse> call = apiService.getTargetForAsm(getPreferences().getStringPref(P.USER_ID), half, year);
            call.enqueue(new Callback<SelectAsmResponse>() {
                @Override
                public void onResponse(Call<SelectAsmResponse> call, Response<SelectAsmResponse> response) {
                    SelectAsmResponse data = response.body();
